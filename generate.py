...
def create_thumbnail(source_disk_path, year, month, day, filename):
    thumb_dir, thumb_disk_path, thumb_web_path = build_thumb_paths(
        year, month, day, filename
    )

    source_mtime = os.path.getmtime(source_disk_path)
    if (
        os.path.exists(thumb_disk_path)
        and os.path.getmtime(thumb_disk_path) >= source_mtime
    ):
    ...
    for year in sorted(os.listdir(BASE_DIR), reverse=True):
        ...
        for month in sorted(os.listdir(year_path), reverse=True):
            ...
            for day in sorted(os.listdir(month_path), reverse=True):
                ...
                full_day_name = datetime(int(year), int(month), int(day)).strftime(
                    "%A %d %B %Y"
                )
                ...
                for file in sorted(os.listdir(day_path)):
                    ...
                    thumb_path = create_thumbnail(
                        source_disk_path, year, month, day, file
                    )
                    caption_path = os.path.join(
                        day_path, file.rsplit(".", 1)[0] + ".txt"
                    )
                    caption = ""
...
def generate_album_html(months):
    ...
    if months:
        ...
        jump_links = [
            f'<a href="#{month["anchor"]}">{month["title"]}</a>' for month in months
        ]
    ...
    for month in months:
        ...
        for day in month["days"]:
            ...
            if day["note"]:
                html_parts.append(
                    f'<p class="album-day-note">{to_html_text(day["note"])}</p>'
                )

            html_parts.append('<div class="album-grid">')
            for image in day["images"]:
                escaped_caption = (
                    to_html_text(image["caption"])
                    if image["caption"]
                    else '<span class="album-empty-caption">No caption.</span>'
                )
